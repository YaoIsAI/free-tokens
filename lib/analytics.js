...
function recordVisit({ ip, ua, path, tz, lang, ref } = {}) {
  try {
    ...
    const n = db.prepare('SELECT COUNT(*) c FROM visits').get().c;
    if (n > MAX_VISITS) {
      db.prepare('DELETE FROM visits WHERE id IN (SELECT id FROM visits ORDER BY id ASC LIMIT ?)').run(n - MAX_VISITS);
    }
