...
function getDb() {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      pull_count INTEGER DEFAULT 0 CHECK(pull_count >= 0),
    ...
    -- 邮箱验证码（绑定/改绑/重置）：6 位数字 + bcrypt 哈希 + 10 分钟过期 + 单次使用
    CREATE TABLE IF NOT EXISTS email_codes (
      id TEXT PRIMARY KEY,
      user_id TEXT DEFAULT '',
      email TEXT NOT NULL,
      purpose TEXT NOT NULL CHECK(purpose IN ('bind','reset','change')),
    ...
    -- 邮箱重置链接 token：32 字节随机 + sha256 存表 + 30 分钟过期 + 单次有效
    ...
    CREATE INDEX IF NOT EXISTS idx_email_resets_user ON email_resets(user_id, created_at);

    -- 邮件服务配置（仅一行 id='default'）：host/port/user/pass/from；后端启动时若空则邮件功能关闭
    -- pass 存 AES-GCM 密文（env: EMAIL_CONFIG_KEY），前端 /api/admin/email 仅返回脱敏
    CREATE TABLE IF NOT EXISTS email_settings (
      ...
      user TEXT DEFAULT '',
      pass_cipher TEXT DEFAULT '',
      pass_iv TEXT DEFAULT '',
      pass_tag TEXT DEFAULT '',
    ...
    INSERT OR IGNORE INTO email_settings(id, enabled) VALUES('default', 0);

    -- 邮件发送流水（运营/版主审计：时间/收件人/主题/用途/结果/IP；密码/token/验证码明文不存）
  ...
  // 迁移：其余幂等奖励（invite/item_verify/tutorial_verify/skill_verify 等 7 类）DB 级兜底防并发双发
  try {
    db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_points_log_idempotent ON points_log(user_id, reason, ref_id) WHERE reason IN ('item_verify','tutorial_verify','skill_verify','skill_first_sale','skill_review','feedback_verify','invite')`);
  } catch (e) { /* ignore */ }

  // 迁移：邮箱验证码/重置链接 token 表
  try {
    db.exec(`CREATE TABLE IF NOT EXISTS email_codes (id TEXT PRIMARY KEY, user_id TEXT DEFAULT '', email TEXT NOT NULL, purpose TEXT NOT NULL CHECK(purpose IN ('bind','reset','change')), code_hash TEXT NOT NULL, expires_at TEXT NOT NULL, used_at TEXT DEFAULT '', attempts INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime('now')))`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_email_codes_email ON email_codes(email, purpose, created_at)`);
  } catch (e) { /* ignore */ }
  try {
    db.exec(`CREATE TABLE IF NOT EXISTS email_resets (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, email TEXT NOT NULL, token_hash TEXT NOT NULL, expires_at TEXT NOT NULL, used_at TEXT DEFAULT '', ip TEXT DEFAULT '', ua TEXT DEFAULT '', created_at TEXT NOT NULL DEFAULT (datetime('now')))`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_email_resets_user ON email_resets(user_id, created_at)`);
  } catch (e) { /* ignore */ }
  try {
    db.exec(`CREATE TABLE IF NOT EXISTS email_settings (id TEXT PRIMARY KEY DEFAULT 'default', host TEXT DEFAULT '', port INTEGER DEFAULT 465, secure INTEGER NOT NULL DEFAULT 1, user TEXT DEFAULT '', pass_cipher TEXT DEFAULT '', pass_iv TEXT DEFAULT '', pass_tag TEXT DEFAULT '', from_addr TEXT DEFAULT '', enabled INTEGER NOT NULL DEFAULT 0, last_test_at TEXT DEFAULT '', last_test_msg TEXT DEFAULT '', updated_by TEXT DEFAULT '', updated_at TEXT NOT NULL DEFAULT (datetime('now')))`);
    db.exec(`INSERT OR IGNORE INTO email_settings(id, enabled) VALUES('default', 0)`);
  } catch (e) { /* ignore */ }
  try {
    db.exec(`CREATE TABLE IF NOT EXISTS email_logs (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id TEXT DEFAULT '', email TEXT NOT NULL, purpose TEXT NOT NULL, subject TEXT NOT NULL, ok INTEGER NOT NULL DEFAULT 0, err TEXT DEFAULT '', ip TEXT DEFAULT '', created_at TEXT NOT NULL DEFAULT (datetime('now')))`);
    db.exec(`CREATE INDEX IF NOT EXISTS idx_email_logs_created ON email_logs(created_at)`);
  } catch (e) { /* ignore */ }
  // 迁移：users 补 email_verified_at 列（首次邮箱绑定成功后写入；后续重置密码需此列非空）
  try {
    const ucols = db.prepare('PRAGMA table_info(users)').all();
    if (!ucols.some(c => c.name === 'email_verified_at')) {
      db.exec(`ALTER TABLE users ADD COLUMN email_verified_at TEXT DEFAULT ''`);
    }
  } catch (e) { /* ignore */ }
