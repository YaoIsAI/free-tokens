...
// 幂等发放：同 (user, reason, refId) 已发放过则跳过（发布即发分后，"下线→重新通过"不再重复发放）
function awardPointsOnce(userId, delta, reason, refId) {
  if (!userId) return;
  const db = getDb();
  const exists = db.prepare('SELECT id FROM points_log WHERE user_id = ? AND reason = ? AND ref_id = ?')
    .get(userId, reason || '', refId || '');
  if (exists) return;
  awardPoints(userId, delta, reason, refId);
}
...
// 拉取计费：先耗每日免费额度，超出部分扣积分；返回统计或 { error }
function consumePulls(userId, count, isStaff) {
  ...
  // 版主/管理员豁免：不计次、不扣分
  if (isStaff) {
    return { freeUsed: count, pointsUsed: 0, pointsRemaining: user.points || 0, freeLeftToday: FREE_PULLS_PER_DAY };
  }
  ...
  const need = count - freeUsed; // 需扣积分的次数
  if (need > points) {
    return { error: `积分不足：本次需 ${need} 积分，当前 ${points} 分；发布 Token/教程/Skill、邀请、测评可得积分` };
  }
  ...
  db.transaction(() => {
    ...
    db.prepare('UPDATE users SET points = ?, pull_date = ?, pull_count = ? WHERE id = ?')
      .run(newPoints, today, newCount, userId);
  })();
  ...
  return {
    ...
    freeLeftToday: Math.max(0, FREE_PULLS_PER_DAY - newCount)
  };
}
