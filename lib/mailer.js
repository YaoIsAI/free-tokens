...
function saveConfig({ host, port, secure, user, pass, fromAddr, enabled, updatedBy }) {
  const db = getDb();
  const cur = readConfig();
  // 密码留空则沿用旧密文（不改）
  let enc;
  if (pass) enc = encryptSecret(pass);
  else if (cur.pass) {
    const row = getDb().prepare('SELECT pass_cipher, pass_iv, pass_tag FROM email_settings WHERE id=?').get('default') || {};
    enc = { cipher: row.pass_cipher || '', iv: row.pass_iv || '', tag: row.pass_tag || '' };
  } else enc = { cipher: '', iv: '', tag: '' };
  db.prepare(`UPDATE email_settings SET
    host=?, port=?, secure=?, user=?, pass_cipher=?, pass_iv=?, pass_tag=?, from_addr=?, enabled=?, updated_by=?, updated_at=datetime('now')
